...
class solver_CCD(read_inputdata):
    ...
    def ccd_calc(self):
        ...
        scenariosLength = int(len(dfvarCol[dfvarColKey[0]][0]))
        ...
        for k in range(len(caseStudyStns)):       
            ...
            for j in range(climateModel, climateModel + 1, 1):
                ...
                for i in range(len(self.args)):#i for variable Tmax or Tmin k for stations (0t.csv and 1t.csv), j for climate model meaning the clomun of csv file
                    todayVar[todayVar_keys[i]] = round(dfvar[dfvarkeys[k*len(self.args)+i]][0][dfvarCol[dfvarColKey[k*len(self.args)+i]][0][j-1]].iloc[1], 2) \
                    if (dfvar[dfvarkeys[k*len(self.args)+i]][0][dfvarCol[dfvarColKey[k*len(self.args)+i]][0][j-1]].iloc[1] != -99) else 0
            ...
            for k in range(len(caseStudyStns)):       
                ...
                for j in range(climateModel, climateModel + 1, 1):
                    ...
                    for i in range(len(self.args)):#i for variable Tmax or Tmin k for stations (0t.csv and 1t.csv), j for climate model meaning the clomun of csv file
                        ...
                        todayVar[todayVar_keys[i]] = round(dfvar[dfvarkeys[k*len(self.args)+i]][0][dfvarCol[dfvarColKey[k*len(self.args)+i]][0][j-1]].iloc[t], 2) \
                        if (dfvar[dfvarkeys[k*len(self.args)+i]][0][dfvarCol[dfvarColKey[k*len(self.args)+i]][0][j-1]].iloc[t] != -99) else 0
...
root = r'C:\Saeid\Prj100\SA_47_CCHDNs_package\data\Zurich_kloten_v3' 
