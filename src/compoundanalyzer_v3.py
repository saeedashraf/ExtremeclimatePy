        ...
        if (xClimateRandomness == 1):
            for i in range(len(_VarCaseStudy)):
                ...
                for key, value in varCaseStudy.items():
                    varCaseStudy[key].append(os.path.join(climateFolder_org, caseStudyStns[i]['fileName'] + str() + '.csv'))

        if (xClimateRandomness == 2):
            for i in range(len(_VarCaseStudy)):
                for key, value in varCaseStudy.items():
                    varCaseStudy[key].append(os.path.join(climateFolder1, caseStudyStns[i]['fileName'] + str() + '.csv'))

        if (xClimateRandomness == 3):
            for i in range(len(_VarCaseStudy)):
                for key, value in varCaseStudy.items():
                    varCaseStudy[key].append(os.path.join(climateFolder2, caseStudyStns[i]['fileName'] + str() + '.csv'))
