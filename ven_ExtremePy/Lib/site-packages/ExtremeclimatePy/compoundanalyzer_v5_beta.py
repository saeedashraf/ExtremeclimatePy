...
class read_inputdata:
    ...
    def _initiate_dict(self, *args, **kwargs):
        ...
        tempdict = {self.arg: i for i, self.arg in enumerate(self.args)}
    ...
    def _initialize_input_dict(self):
        ...
        climate_ref_Folder = os.path.join(inputFolder, 'Climate_ref_no_randomness_0')
        ...
        operator_Var = [ 'opr_' + (str(self.args[i])) for i in range (len(self.args))]

        for i in range(len(self.args)):
            os.chdir(varFolder[i])                
            with open(filesA[i][0], 'r') as file:
                operator_Var[i] = file.read()
            ...
            operator_Var[i] = operator_Var[i].replace('\n', '\t').split('\t')
       ...
       # _temp_ValVar= ['ValTmax', 'ValTmin']
        ...
        _temp_operator_Var = ['Opertor_' + (str(self.args[i])) for i in range (len(self.args))]

        for i in range(len(self.args)):
            for j, element in enumerate(operator_Var[i]):
                for k in range (len(nameStn)):
                    if element == stnDicts[k]['fileName']:
                        print(element)
                        print(operator_Var[i][j+1])
                        stnDicts[k][_temp_operator_Var[i]] = operator_Var[i][j+1]
...
class RCP_Model:
    def __init__(self, xRCP, xClimateModel, nObs, nRCP26, nRCP45, nRCP85):
...
class solver_CCD(read_inputdata):
    ...
    def __init__(self, root, start_date, end_date, xRCP, xClimateModel, nObs, nRCP26, nRCP45, nRCP85, deepuncertainty_rand, *args, **kwargs):
    ...
    def ccd_calc(self):
        ...
        final_var = collections.OrderedDict()
        is_final_var_extremes = collections.OrderedDict()
        len_final_var_extremes = collections.OrderedDict()
        ...
        for k in range(len(caseStudyStns)):
            ...
            final_var['final_var_stn_' + "{0}".format(str(k))] = [0 for _ in range(simulationLength)]
            is_final_var_extremes['is_final_var_compound_stn_' + "{0}".format(str(k))] = [0 for _ in range(simulationLength)]
            len_final_var_extremes['len_final_var_stn_' + "{0}".format(str(k))] = [0 for _ in range(simulationLength)]
            total['total_' + "{0}".format(str(k))] = np.zeros([simulationLength, (2*len(self.args) + 2 + 3)]) #2 for is_ and len_ of compund and three for final_var, is extreme, len
        ...
        final_var_keys = list(final_var.keys())
        is_final_var_extremes_keys = list(is_final_var_extremes.keys())
        len_final_var_extremes_keys = list(len_final_var_extremes.keys())
        ...
        rcp_Model = RCP_Model(self.xRCP, self.xClimateModel, self.nObs, self.nRCP26, self.nRCP45, self.nRCP85)
        ...
        for k in range(len(caseStudyStns)):       
            ...
            for j in range(climateModel, climateModel + 1, 1):
                ...
                for i in range(len(self.args)):#i for variable Tmax or Tmin k for stations (0t.csv and 1t.csv), j for climate model meaning the clomun of csv file
                    ...
                    #20230315
                    expression = f"todayVar[todayVar_keys[i]] {dic_operator[caseStudyStns[k][caseStudyStns_keys[len(self.args) + 1 + 3*i]]]} float(caseStudyStns[k][caseStudyStns_keys[len(self.args) + 3 + 3*i]])"
                ...
                # modify here
                #final_var = todayVar[todayVar_keys[0]] + todayVar[todayVar_keys[1]] + todayVar[todayVar_keys[2]] + todayVar[todayVar_keys[2]] + \
                #todayVar[todayVar_keys[3]] +  todayVar[todayVar_keys[4]] +  todayVar[todayVar_keys[5]] 
        ...
        for t in range(2, simulationLength + 1, 1):
            for k in range(len(caseStudyStns)):       
                ...
                for j in range(climateModel, climateModel + 1, 1):
                    ...
                    total[total_keys[k]][t-1,(2*len(self.args) + 4)] = len_final_var_extremes[len_final_var_extremes_keys[k]][t-1]
        ...
        for k in range(len(caseStudyStns)):
            ...
            #modify here 20230316
            columnsDF.append('Final_VAR_' + nameHeader)
            columnsDF.append('Is_final_Var_exEve_' + nameHeader)
            columnsDF.append('Length_final_VARs_exEve_' + nameHeader)
...
# root = r'C:\Saeid\Prj100\SA_47_CCHDNs_package\data\case_study_3_Basel_WBGT'
